...
const dragGraph = function(
  { x, y, w, h, type, text, fontSize = 20, color = "red", url, count = 0 },
  canvas,
  factor
) {
  if (type === "text") {
...
dragGraph.prototype = {
  ...
  isInGraph(x, y) {
    const selectW =
      this.type === "text" ? this.ctx.measureText(this.text).width : this.w;
    ...
    const transformDelX =
      this._getTransform(
        delX,
        delY,
        this.rotate - this._getAngle(this.centerX, this.centerY, delX, delY)
      ).x -
      delW / 2;
    const transformDelY =
      this._getTransform(
    ...
    const scaleH = 30;
    const scaleX =
      this.type === "text" ? this.x + selectW / 2 : this.x + selectW;
    const scaleY =
      this.type === "text" ? this.y + selectH / 2 : this.y + selectH;
    // 旋转后的变换区域坐标
    const transformScaleX =
      this._getTransform(
        scaleX,
        scaleY,
        this.rotate + this._getAngle(this.centerX, this.centerY, scaleX, scaleY)
      ).x -
      scaleW / 2;
  ...
  transform(px, py, x, y, currentGraph) {
    ...
    if (this.type === "image") {
      ...
      const h =
        currentGraph.h + (lineB - lineA) * (currentGraph.h / currentGraph.w);
...
Component({
  ...
  methods: {
    ...
    onGraphChange(n, o) {
      ...
      if (this.drawArr === undefined) return;
      this.drawArr.push(
        new dragGraph(
          Object.assign(
            {
